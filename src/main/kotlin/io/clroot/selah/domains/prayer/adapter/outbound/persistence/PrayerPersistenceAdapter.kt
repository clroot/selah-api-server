...
class PrayerPersistenceAdapter(
    ...
    override suspend fun findById(id: PrayerId): Prayer? =
        sessionFactory
            .withSession { session ->
                session.find(PrayerEntity::class.java, id.value)
            }.awaitSuspending()
            ?.let { mapper.toDomain(it) }
    ...
    override suspend fun findByIdAndMemberId(
        ...
        sessionFactory
            .withSession { session ->
                session
                    .createQuery(
                        "from PrayerEntity where id = :id and memberId = :memberId",
                        PrayerEntity::class.java,
    ...
    override suspend fun findAllByMemberId(
        ...
        sessionFactory
            .withSession { session ->
                session
                    .createQuery(
                        "select count(e) from PrayerEntity e where e.memberId = :memberId",
                        Long::class.java,
                    ).setParameter("memberId", memberId.value)
                    .singleResult
                    .chain { total: Long ->
                        session
                            .createQuery(
                                "from PrayerEntity where memberId = :memberId order by createdAt desc",
                                PrayerEntity::class.java,
                            ).setParameter("memberId", memberId.value)
