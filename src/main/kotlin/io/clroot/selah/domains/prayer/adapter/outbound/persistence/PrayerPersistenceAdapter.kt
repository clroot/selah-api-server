...
class PrayerPersistenceAdapter(
...
) : SavePrayerPort, LoadPrayerPort, DeletePrayerPort {

    override suspend fun save(prayer: Prayer): Prayer = withContext(Dispatchers.IO) {
        val savedEntity = if (repository.existsById(prayer.id.value)) {
    ...
    override suspend fun findById(id: PrayerId): Prayer? = withContext(Dispatchers.IO) {
        repository.findById(id.value).orElse(null)?.let { mapper.toDomain(it) }
    }

    override suspend fun findByIdAndMemberId(id: PrayerId, memberId: MemberId): Prayer? =
    ...
    override suspend fun findAllByMemberId(
        memberId: MemberId,
        pageable: Pageable,
    ): Page<Prayer> = withContext(Dispatchers.IO) {
        // Count query
        val countQuery = jpql {
    ...
    override suspend fun deleteById(id: PrayerId) = withContext(Dispatchers.IO) {
