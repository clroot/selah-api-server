...
/**
 * 이메일 인증 토큰 관리를 위한 Outbound Port
 *
...
interface EmailVerificationTokenPort {
     ...
     */
    suspend fun create(memberId: MemberId): EmailVerificationTokenCreateResult

    /**
     * 토큰으로 유효한 인증 정보를 조회합니다.
     * 만료되거나 사용된 토큰은 반환되지 않습니다.
     *
     * @param token 원본 토큰
     * @return 토큰 정보 또는 null
     */
    suspend fun findValidByToken(token: String): EmailVerificationTokenInfo?
 ...
 */
data class EmailVerificationTokenCreateResult(
    val info: EmailVerificationTokenInfo,
    val rawToken: String,
)
