...
class ServerKeyPersistenceAdapter(
...
) : LoadServerKeyPort, SaveServerKeyPort, DeleteServerKeyPort {

    override suspend fun findByMemberId(memberId: MemberId): ServerKey? = withContext(Dispatchers.IO) {
    ...
    override suspend fun save(serverKey: ServerKey): ServerKey = withContext(Dispatchers.IO) {
        val savedEntity = if (repository.existsById(serverKey.id.value)) {
            // 기존 Entity 업데이트
