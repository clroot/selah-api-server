...
class LoginServiceTest : DescribeSpec({
    ...
    describe("loginWithEmail") {
        ...
        val command = LoginWithEmailCommand(
            email = email,
        ...
        context("비밀번호가 없는 OAuth 전용 회원이 이메일 로그인을 시도할 때") {
            val oauthOnlyMember = createOAuthOnlyMember(
                id = memberId,
                email = email,
            )
        ...
        context("이메일이 인증되지 않은 회원이 로그인할 때") {
            val unverifiedMember = createMember(
                id = memberId,
                email = email,
                passwordHash = passwordHash,
                emailVerified = false,
            ...
            it("EmailNotVerifiedException을 던진다") {
                ...
                val exception = shouldThrow<EmailNotVerifiedException> {
                    loginService.loginWithEmail(command)
                }
    ...
    describe("loginWithOAuth") {
        ...
        val command = LoginWithOAuthCommand(
            email = email,
            nickname = nickname,
            provider = provider,
        ...
        context("신규 회원이 OAuth로 로그인할 때") {
            val memberId = MemberId.new()
            val newMember = createOAuthOnlyMember(
                id = memberId,
                email = email,
                nickname = nickname,
            )
        ...
        context("기존 회원이 OAuth로 로그인할 때") {
            val memberId = MemberId.new()
            val existingMember = createOAuthOnlyMember(
                id = memberId,
                email = email,
