...
@TestPropertySource(
    properties = [
        "selah.email-verification.ttl=PT1H",
    ],
)
class EmailVerificationTokenPersistenceAdapterTest : IntegrationTestBase() {
    @Autowired
    ...
    init {
        describe("EmailVerificationTokenPersistenceAdapter") {
            ...
            describe("EmailVerificationTokenInfo") {
                context("isExpired") {
                    it("만료 여부를 정확히 판단한다") {
                        // Given
                        val created = adapter.create(testMember.id)
