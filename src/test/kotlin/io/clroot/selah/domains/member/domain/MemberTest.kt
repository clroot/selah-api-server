...
class MemberTest : DescribeSpec({
    ...
    describe("OAuth 관리") {
        ...
        context("OAuth 연결") {
            ...
            it("여러 Provider를 연결할 수 있다") {
                ...
                member.connectedProviders shouldContainExactly setOf(
    ...
    describe("Computed Properties") {
        ...
        it("hasPassword는 비밀번호 존재 여부를 반환한다") {
            val memberWithPassword = Member.createWithEmail(
                email = Email("test@example.com"),
                nickname = "테스트유저",
                passwordHash = PasswordHash.from("hashed_password"),
            )
            val memberWithoutPassword = Member.createWithOAuth(
                email = Email("test@example.com"),
                nickname = "테스트유저",
                provider = OAuthProvider.GOOGLE,
                providerId = "google_123",
        ...
        it("hasOAuthConnection은 OAuth 연결 존재 여부를 반환한다") {
            val memberWithOAuth = Member.createWithOAuth(
                email = Email("test@example.com"),
                nickname = "테스트유저",
                provider = OAuthProvider.GOOGLE,
                providerId = "google_123",
            )
            val memberWithoutOAuth = Member.createWithEmail(
                email = Email("test@example.com"),
                nickname = "테스트유저",
                passwordHash = PasswordHash.from("hashed_password"),
            )
